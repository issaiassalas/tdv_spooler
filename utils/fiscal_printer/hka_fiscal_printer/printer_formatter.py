...
class TDVHKAFormatter:
    ...
    def custom_invoice(self,
        ...
        return self.factura(products=products, payments=payments)
    ...
    def credit_note(self, 
        ...
        self.print_invoice_products(products=products, invoice_type='CREDIT')
    ...
    def get_tax_type(self, tax: str = 'None', invoice_type='DELIVERY'):
        if invoice_type in ['DELIVERY', 'delivery']:
            if tax.startswith('IVA'):
                return '!'
            elif tax.startswith('IVAR'):
                return '"'
            elif tax.startswith('IVAL'):
                return '#'
            elif tax == 'None':
                return ' '
        elif invoice_type in ['CREDIT', 'credit']:
            if tax.startswith('IVA'):
                return 'd1'
            elif tax.startswith('IVAR'):
                return 'd2'
    ...
    def print_invoice_products(self,
        ...
        for product in products:
            ...
            line += self.format_units(unit=product.get('price_unit'),
                                     unit_type='price')
            line += self.format_units(unit=product.get('qty'),
                                     unit_type='quantity')
            line += product.get('full_product_name')[:33]
            if not DEBUG:
